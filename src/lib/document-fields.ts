...
export const documentFields: { [key: string]: DocumentField[] } = {
  ...
  'contrat-de-pret-personnel': [
    ...
    { name: 'loan_amount', label: {fr: 'Montant du prêt (€)', en: 'Loan Amount (€)'}, type: 'number', validation: { type: 'number' } },
    { name: 'loan_amount_in_words', label: {fr: 'Montant en toutes lettres', en: 'Amount in words'}, type: 'text', validation: { type: 'string' } },
    { name: 'taeg', label: {fr: 'TAEG Fixe', en: 'Fixed APR'}, type: 'text', defaultValue: '2.00%', validation: { type: 'string' } },
    { name: 'loan_term', label: {fr: 'Durée du prêt (mois)', en: 'Loan Term (months)'}, type: 'number', validation: { type: 'number' } },
  ...
  'recu-de-paiement': [
    { name: 'ref', label: { fr: 'Référence du reçu', en: 'Receipt Reference' }, type: 'text', defaultValue: `RECU-${new Date().getFullYear()}-`, validation: { type: 'string' } },
    { name: 'payment_date', label: { fr: 'Date du paiement', en: 'Payment Date' }, type: 'date', defaultValue: new Date().toISOString().split('T')[0], validation: { type: 'date' } },
    { name: 'payer_name', label: { fr: 'Nom du payeur', en: 'Payer Name' }, type: 'text', validation: { type: 'string' } },
    { name: 'payer_address', label: { fr: 'Adresse du payeur', en: 'Payer Address' }, type: 'text', validation: { type: 'string' } },
    { name: 'payment_amount', label: { fr: 'Montant reçu (€)', en: 'Amount Received (€)' }, type: 'number', validation: { type: 'number' } },
    { name: 'payment_amount_in_words', label: { fr: 'Montant en toutes lettres', en: 'Amount in Words' }, type: 'text', validation: { type: 'string' } },
    { name: 'payment_method', label: { fr: 'Moyen de paiement', en: 'Payment Method' }, type: 'text', defaultValue: 'Virement bancaire', validation: { type: 'string' } },
    { name: 'payment_reference', label: { fr: 'Référence du paiement (ex: facture, contrat)', en: 'Payment Reference (e.g., invoice, contract)' }, type: 'text', validation: { type: 'string' } },
  ],
  'document-vierge': [],
};
