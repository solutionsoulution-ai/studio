...
const neofondsInvoiceFields: DocumentField[] = [
  ...
  { name: 'client_name', label: {fr:'Nom du client', en: 'Client Name', de: 'Kundenname'}, type: 'text', validation: { type: 'string' } },
  { name: 'client_address', label: {fr:'Adresse du client', en: 'Client Address', de: 'Kundenadresse'}, type: 'textarea', validation: { type: 'string' } },
  { name: 'vat_rate', label: {fr:'Taux de TVA (%)', en: 'VAT Rate (%)', de: 'MwSt.-Satz (%)'}, type: 'number', defaultValue: 19, validation: { type: 'number' } },
  { name: 'items_group', label: {fr: "Détails de la facturation", en: "Billing Details", de: "Rechnungsdetails"}, type: 'group', validation: {type: 'any'}, fields: [
    { name: 'item1_description', label: {fr: 'Article 1 - Description'}, type: 'text', validation: { type: 'string' } },
    { name: 'item1_quantity', label: {fr: 'Article 1 - Quantité'}, type: 'number', defaultValue: 1, validation: { type: 'number' } },
    { name: 'item1_unit_price', label: {fr: 'Article 1 - Prix U.'}, type: 'number', validation: { type: 'number' } },
...
const genericReceiptFields: DocumentField[] = [
...
export const documentFields: { [key: string]: DocumentField[] } = {
  'reconnaissance-de-dette': [
    ...
    { name: 'loan_amount_in_words', label: {fr: 'Montant en toutes lettres (Euros)', en: 'Amount in words (Euros)', de: 'Betrag in Worten (Euro)'}, type: 'text', validation: { type: 'string' } },
  ...
  'contrat-de-pret-personnel': [
    ...
    { name: 'loan_amount', label: {fr: 'Montant du prêt'}, type: 'number', defaultValue: 30000, validation: { type: 'number' } },
    ...
    { name: 'loan_term', label: {fr: 'Durée du prêt (mois)'}, type: 'number', defaultValue: 120, validation: { type: 'number' } },
    { name: 'availability_days', label: {fr: 'Jours avant mise à disposition'}, type: 'number', defaultValue: 10, validation: { type: 'number' } },
    { name: 'start_date', label: {fr: 'Date de 1ère échéance'}, type: 'date', defaultValue: new Date().toISOString().split('T')[0], validation: { type: 'date' } },
    { name: 'monthly_payment', label: {fr: 'Mensualité'}, type: 'number', defaultValue: 276.01, validation: { type: 'number' } },
    { name: 'total_cost', label: {fr: 'Coût total du crédit'}, type: 'number', defaultValue: 3121.20, validation: { type: 'number' } },
    { name: 'total_due', label: {fr: 'Montant total dû'}, type: 'number', defaultValue: 33648.20, validation: { type: 'number' } },
    { name: 'reimbursed_fees', label: {fr: 'Frais à rembourser'}, type: 'number', defaultValue: 527, validation: { type: 'any' } },
