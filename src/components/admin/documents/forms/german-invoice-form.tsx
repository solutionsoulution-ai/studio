...
const formSchema = z.object({
    ...
    customer_address: z.string().min(5, "Kundenadresse ist erforderlich."),
    invoice_number: z.string().min(3, "Rechnungsnummer ist erforderlich."),
    invoice_date: z.string().min(8, "Rechnungsdatum ist erforderlich."),
    description: z.string().min(5, "Beschreibung ist erforderlich."),
    amount: z.coerce.number().positive("Betrag muss positiv sein."),
    payment_iban: z.string().min(15, "Eine gültige IBAN ist erforderlich."),
});
    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>Füllen Sie die Felder aus, um die deutsche Rechnung zu erstellen.</CardDescription>
            </CardHeader>
            <CardContent>
                <Form {...form}>
                    <form className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                           <FormField control={form.control} name="invoice_number" render={({ field }) => (
                                <FormItem>
                                    <FormLabel>Rechnungsnr.</FormLabel>
                                    <FormControl><Input {...field} /></FormControl>
                                    <FormMessage />
                                </FormItem>
                            )} />
                        ...
                        <FormField control={form.control} name="payment_iban" render={({ field }) => (
                            <FormItem>
                                <FormLabel>Zahlungs-IBAN</FormLabel>
                                <FormControl><Input {...field} /></FormControl>
                                <FormMessage />
                            </FormItem>
                        )} />
