  ...
  webpack: (config, { isServer, dev }) => {
    if (!isServer && !dev) {
      ...
      config.entry = async () => {
        ...
        // This handles the main page for WordPress export
        if (entries['app/page']) {
           entries['main'] = entries['app/page'];
           delete entries['app/page'];
        }

        // This ensures the dynamic document pages are also processed
        const docPageKey = Object.keys(entries).find(key => key.includes('app/documents/[slug]/page'));
        if (docPageKey) {
          entries['document-generator'] = entries[docPageKey];
          // We don't delete the original because Next.js needs it for routing
        }
